...
% hl    header lines (as a char vector)
...
% fmt   format string
% SR    cell array of replacement texts  sr<i>, SR = {sr1,sr2,...}
% SX    cell array of invalid line texts sx<i>, SX = {sx1,sx2,...}
%
...
%
% Additional character vectors <sr1>,<sr2>,.. can be supplied within a cell
% array <SR> to perform single character substitution before the data is
% converted: each of the first n-1 characters of an <n>-element character
% vector is replaced by the <n>-th character.
% See also remark 1) for the interchangeability of character type and
% string type inputs. 
%
% A further optional input argument is a cell array <SX> containing
% character vectors <sx1>,<sx2>,.. that mark "bad" lines containing invalid
% data. If every line containing invalid data can be caught by the <SX>,
% TXT2MAT will speed up significantly (see EXAMPLE 3a). Any lines that are
% recognized to be invalid are completely ignored (and there is no
% corresponding row in A). 
%
% If the number of header lines <nh> or the number of data columns <nc> are
% not provided, TXT2MAT performs an automatic analysis of the file format.
...
% positional argument syntax txt2mat(ffn,nh,nc,fmt,SR,SX). For usage see
% EXAMPLE 3a.
% The following table lists the param/value-pairs and their corresponding
% single argument, if existing:
%
%  Param             Value type  Example value                  single arg.
...
%  'MemPar'          Scalar      2^17                                     -
%
% The param/value-pairs may follow the positional arguments in any order,
% e.g. txt2mat('file.txt',13,9,'BadLineString',{'Bad'},'Format','%f'). Only
...
%   cell array containing at least one cell array of characters must be
%   provided. Such a cell array of characters consists of <n> char vectors
%   with n>=2, so that each of the first <n-1> character sequences is
%   replaced by the <n>-th one. 
%   For example, with {{'R1a','R1b, 'S1'}, {'R2a','R2b','R2c', 'S2'}}
%   all the 'R<n>'-sequences are replaced by the corresponding 'S<n>' one.
...
%             cause an error (see also remark 3).
...
%   'cell'    Same as 'char', but put the lines of text successively into 
%             separate elements of a cell vector, see EXAMPLE 3b.
%   'string'  Same as 'cell', but put the lines of text successively into 
%             separate elements of a string vector.
%
...
% REMARKS
%
% 1) You may use string arrays in place of cell arrays of character
%    vectors ( ["True","1"] <-> {'True','1'} ) and string scalars in place
%    of stand-alone character vectors ( "Format" <-> 'Format' ) as input
%    arguments. 
%
% 2) Prerequisites for the automatic file format analysis before the
...
%
% 3) In matrix mode, txt2mat checks that the format string is suitable
...
% -------------------------------------------------------------------------
%
% A = txt2mat;      % choose a file and let TXT2MAT analyze its format
%                 
% =========================================================================
...
% the regular expression replacement. In this way you can use the latter
% command on similar files which have a different or previously unknown
% number of header lines etc., too.
%
% =========================================================================
...
%   See also SSCANF
% --- Author: -------------------------------------------------------------
%   Copyright 2005-2021 Andres
%   $Revision: 7.00 $  $Date: 2021/10/25 10:25:05 $
% --- E-Mail: -------------------------------------------------------------
% x=-2:3;
...
%   · changed some waitbar checks, removed some typos, use addParameter
%   · removed p-code hint from documentation  
% Extra remark (removed from documentation)
% 4) Since txt2mat.m is a comparatively large file, generating a preparsed
...
% 0) check make sure FileName can be interpreted as char 
if isstring(ffn)
    if numel(ffn)<=1
        ffn = char(ffn);
    else
        error('txt2mat:invalidFileNameString',...
            'multi-element string arrays are not accepted as file name'); 
    end
end
if ~isempty(ffn) && ~ischar(ffn)
    error('txt2mat:invalidFileNameType',...
            'non-empty file name must be a string scalar or char array');
end
...
% switch off file analysis if read mode is 'char','cell', or 'string'
doAnalyzeFile = doAnalyzeFile && ~strcmpi(arg.val.ReadMode,'string') && ...
    ~strcmpi(arg.val.ReadMode,'cell') && ~strcmpi(arg.val.ReadMode,'char');
if doAnalyzeFile 
    ...
    if arg.val.InfoLevel >= 1
        ...
        if numel(anaFileErr)==0
            ...
            disp(['* ' num2str(anaNumAnalyzed)        ' data lines analyzed' ]);
...
%% ReadMode 'cell' or 'string': return lines in a cell or string vector
if strcmpi(readMode,'cell') || strcmpi(readMode,'string')
    ...
    elseif strcmpi(readMode,'string')
        A = repmat("",num_lf-1,1);
        % or A = repmat(string(missing),num_lf-1,1);
        for m = 1:num_lf-1
            A(m) = string(f8(idcLb(m)+1:idcLb(m+1)-numLbfull));
        end
        % not yet tried:
        % A = arrayfun(@(m,n) string(f8(m:n)), ...
        %                         lf_idc(1:end-1)+1,lf_idc(2:end)-num_lbfull);
    end
    if hasWaitbar
        close(hw)
        ...
        switch errorType
            case 'sscanf'
                if (arg.val.InfoLevel >= 2) && ( nextindex <= endIdcF8 - startIdcF8 + 1 )  
                    ...
                    errorLineText = char(f8(startIdcF8 + (idcLbCurr(idxErrorLine-1):idcLbCurr(idxErrorLine)-numLbfull-1) )).';
...
function [anaNumHeader, anaNumColon, anaFormat, anaRepChar, anaReadMode, ...
    anaNumAnalyzed, anaHeader, anaFileErr, anaErr] = anatxt(arg)
% ANATXT analyze data layout in a text file for txt2mat
% 
% Usage:
...
% llta          lines analyzed after header
...
% two sequential commas without a (different) separator between them
...
if isnan(numHeader) % ~~~~~ there's no user-supplied number of header lines
    ...
    anaNumAnalyzed = numLb - numHeader;	% number of non-header lines to analyze
...
if hasValidatedArgStruct
    ...
    p.addOptional(  'Format'        , '%f', @(x) isempty(x) || ( (ischar(x)||isStringScalar(x)) && contains(x,'%')) )
    p.addOptional(  'ReplaceChar'   , {}  , @(x) isempty(x) || iscellstr(x) || isstring(x) || ischar(x) )
    p.addOptional(  'BadLineString' , {}  , @(x) isempty(x) || iscellstr(x) || isstring(x) )
    %-- param/value only inputs:
    p.addParameter('SelectLineFun' , {}  , @(x) isa(x,'function_handle'))
    p.addParameter('GoodLineString', {}  , @(x) isempty(x) || iscellstr(x) || isstring(x) )
    p.addParameter('ReplaceStr'    , {}  , @(x) isempty(x) || iscell(x) )
    p.addParameter('ReplaceRegExpr', {}  , @(x) isempty(x) || iscell(x) )
    p.addParameter('NumericType'   , 'double', @(x) ischar(x) || isStringScalar(x) )
    p.addParameter('RowRange'      , [1,Inf] , @(x) isnumeric(x) && (numel(x)==2) )
    p.addParameter('FilePos'       , 0   , @(x) isnumeric(x) && isscalar(x))
    p.addParameter('ReadMode'      , 'auto', @(x) ischar(x) || isStringScalar(x) )
    p.addParameter('DialogString'  , 'Select File', @(x) ischar(x) || isStringScalar(x) )
    ...
    p.addParameter('ConvString'    , '%f', @(x) isempty(x) || ( (ischar(x)||isStringScalar(x)) && contains(x,'%')) )
    ...
    % turn a single string or char array ReplaceChar into a cell of char
    if isstring(arg.val.ReplaceChar)
        arg.val.ReplaceChar = cellstr(arg.val.ReplaceChar);
    elseif ischar(arg.val.ReplaceChar)
    ...
    % ensure Bad/GoodLineString is a cell of char
    if isstring(arg.val.BadLineString)
        arg.val.BadLineString  = cellstr(arg.val.BadLineString);
    end
    if isstring(arg.val.GoodLineString)
        arg.val.GoodLineString = cellstr(arg.val.GoodLineString);
    end
    ...
    % check if ReplaceStr is empty or has correct Find+Replace elements
    if arg.has.ReplaceStr && ~isempty(arg.val.ReplaceStr)
        for k = 1:arg.num.ReplaceStr
            if isstring(arg.val.ReplaceStr{k})
                arg.val.ReplaceStr{k} = cellstr(arg.val.ReplaceStr{k});
            end
            if ~iscellstr(arg.val.ReplaceStr{k}) || ...
                    numel(arg.val.ReplaceStr{k})<2
                error('txt2mat:ReplaceStr', ...
                    ['ReplaceStr must be a cell array of two-or-more-element\n' ... 
                     'cell arrays of character vectors\n',...
                     'or a cell array of two-or-more-element string arrays'])
            end
        end
    end
    
    % check if ReplaceRegExpr is empty or has correct Find+Replace elements
    if arg.has.ReplaceRegExpr && ~isempty(arg.val.ReplaceRegExpr)
        for k = 1:arg.num.ReplaceRegExpr
            if isstring(arg.val.ReplaceRegExpr{k})
                arg.val.ReplaceRegExpr{k} = cellstr(arg.val.ReplaceRegExpr{k});
            end
            if ~iscellstr(arg.val.ReplaceRegExpr{k}) || ...
                    numel(arg.val.ReplaceRegExpr{k})<2
                error('txt2mat:ReplaceRegExpr', ...
                    ['ReplaceRegExpr must be a cell array of two-or-more-element\n' ... 
                     'cell arrays of character vectors\n',...
                     'or a cell array of two-or-more-element string arrays'])
            end
        end
...
%     % check if ReplaceStr is empty or has correct Find+Replace pairs
%     if arg.has.ReplaceStr && ~isempty(arg.val.ReplaceStr) && ~( ...
...
%     end
    ...
    if arg.has.ReadMode
        knownReadModes = ...
            {'matrix','line','auto','block','char','cell','string'};
        if isstring(arg.val.ReadMode)
            arg.val.ReadMode = char(arg.val.ReadMode);
        end
    ...
    if arg.has.NumericType
        ...
        if isstring(arg.val.NumericType)
            arg.val.NumericType = char(arg.val.NumericType);
        end
