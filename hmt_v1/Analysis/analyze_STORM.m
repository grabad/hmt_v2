...
simnames={'1141_com057'};
%%
for k=1:length(simnames)
simnames={'1141_com057'};
meth = csvread([simnames{k},'_me3.csv'],1,0);
acet = csvread([simnames{k},'_ac.csv'],1,0);
...
%%
cell_img2bin = imbinarize(imgaussfilt(sum(imeth+imac,3),1), 0.05);
...
%cell_img2bin= ~bwareaopen(~cell_img2bin, 0000);
...
save(simnames{k},'cell_img2bin','sdis','meth_within','ac_within','ac_within_ac','meth_within_ac','meth_ac_con','ac_meth_con','meth_domain','ac_domain','meth_ac_con_c','ac_meth_con_c','meth_withinc','ac_withinc','ac_within_ac_c','meth_within_ac_c','ratio','diam_m','diam_a','mdens','adens','adistz','mdistz')
%%
clear;
end
